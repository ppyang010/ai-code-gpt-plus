...
- 对于支持视觉输入的供应商兼容协议，后端后续可再将图片附件转换为多模态 `image_url` 内容块透传给模型；当前版本先不进入模型主流程
- 接收到的图片与文件当前需要先上传到阿里云 OSS，再由业务系统记录对象 Key、访问地址和元数据
...
4. 当前版本不再把图片附件送进模型请求，附件仅做上传、回显和消息关联保存；后续接入支持视觉输入的模型后再补多模态透传
...
- 支持聊天输入区直接粘贴图片
- 支持后续扩展普通文件上传
- 接收到的图片与文件先上传到阿里云 OSS，再记录文件元数据
- 图片上传到 OSS 后默认保持私有对象，前端预览和模型访问统一由后端生成临时签名 URL
- 数据库中的 `file_asset.file_url` 保存原始 OSS 地址，便于后续排查、导出或与外部系统对接
- 数据库中的 `file_asset.signed_url` 保存可直接访问的 OSS 签名地址；阿里云 OSS V4 预签名 URL 最长仅支持 7 天
- 上传完成后返回 `fileId`，供 `/api/chat/message/send` 通过 `attachmentIds` 引用
- 当前版本先不在模型请求阶段透传图片附件，避免把未稳定的多模态协议直接带入主链路
- 当前代码已将图片上传链路切到“本地 / OSS”可配置模式；后续新增通用文件上传接口时，直接复用同一套 OSS 配置和对象 Key 规则
...
- 若使用私有桶 + 签名 URL，上传凭证需具备 `PutObject` 权限；若需要服务端进一步探测对象信息，再补充对应读取权限
- 若需要直接点开数据库地址，可将 `signed_url` 的有效期拉到 OSS 支持的上限 7 天；生产仍建议改回较短时效
...
- 本地存储模式下，`fileUrl` / `thumbnailUrl` 返回 `/api/file/content/{fileId}`
- OSS 存储模式下，`fileUrl` / `thumbnailUrl` 仍返回 `/api/file/content/{fileId}` 这种稳定业务地址；真正访问时由后端 302 到临时签名 URL
- OSS 存储模式下，数据库 `file_url` 字段保存原始 OSS 地址，接口响应里的 `fileUrl` 仍返回稳定业务地址
- OSS 存储模式下，数据库 `signed_url` 字段保存可直接访问的签名地址；若签名过期，再由后端重新生成
- 后续若恢复图片透传模型能力，再按对象 Key 生成新的签名 URL 透传给模型，避免 URL 过期后影响后续发送和继续生成
...
- OSS 模式下，`GET /api/file/content/{fileId}` 会 302 重定向到临时签名 URL；本地模式下继续由服务端回放文件流
- 前端可通过文件选择或剪贴板粘贴两种方式触发同一条图片上传链路
- 私有桶模式下无需关闭“阻止公共访问”，签名 URL 由服务端按时效生成即可
