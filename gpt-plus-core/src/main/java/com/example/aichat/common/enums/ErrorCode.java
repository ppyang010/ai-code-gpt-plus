...
public enum ErrorCode {
    ...
    ADMIN_CONTRACT_NOT_IMPLEMENTED(501, "管理后台接口契约已预留，具体逻辑暂未接入"),
    ADMIN_LOGIN_INVALID(401, "管理员账号或密码错误"),
    ADMIN_AUTH_REQUIRED(401, "后台未登录或登录已失效"),
    ADMIN_AUTH_NOT_CONFIGURED(503, "后台鉴权未配置"),
    ADMIN_MODEL_PROVIDER_NOT_FOUND(404, "模型供应商不存在"),
    ADMIN_MODEL_PROVIDER_CODE_DUPLICATE(409, "模型供应商编码已存在"),
    ADMIN_MODEL_CONFIG_NOT_FOUND(404, "模型配置不存在"),
    ADMIN_MODEL_CONFIG_CODE_DUPLICATE(409, "模型编码已存在"),
    ADMIN_PROMPT_TEMPLATE_NOT_FOUND(404, "提示词模板不存在"),
    ADMIN_PROMPT_TEMPLATE_CODE_DUPLICATE(409, "提示词模板编码已存在"),
    ADMIN_PROMPT_TEMPLATE_EMPTY(400, "提示词模板内容不能为空"),
    ADMIN_PROMPT_TEMPLATE_SCOPE_INVALID(400, "提示词模板作用域不合法"),
    ADMIN_SECRET_CRYPTO_NOT_CONFIGURED(503, "后台加密能力未配置"),
