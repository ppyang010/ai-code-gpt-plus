...
- 认证方式：
  - 首版使用登录接口签发本地 HMAC-SHA256 签名 token，不落服务端 token 表。
  - 新增后台 token 解析拦截器，只拦截 `/api/admin/**`，放行 `/api/admin/auth/login`。
  - 密码存储使用 PBKDF2-HMAC-SHA256 哈希字段，不保存明文。
...
- 模板范围：
  ...
  - 表结构建议使用 `template_scope` 区分 `global_append`、`scene_append` 等作用域，而不是继续使用 `mode_code`。
  - 暂不支持按模型、租户、场景分层模板。
...
- 状态管理：
  ...
  - 后台请求封装统一附加后台 token。
  - 前端路由守卫按 `/admin/**` 和 `/admin/login` 区分登录态放行与重定向。
...
- 提示词模板异常：
  - 模板为空或禁用时，聊天主链路回退会话级 / 请求级附加提示词直传能力。
  - 管理后台保存时禁止空模板内容。
- 日志查询压力：
  - 时间范围为空时默认查询近 7 天。
  - pageSize 最大 100。
...
- 安全：
  - 管理员密码使用 PBKDF2-HMAC-SHA256 哈希保存。
  - 后台 token 使用 HMAC-SHA256 签名，签名密钥通过 `app.admin-auth.token-secret` 配置。
  - 供应商 API Key 不明文返回。
  - 登录失败不暴露账号是否存在。
- 性能：
  - 列表接口全部分页。
...
- 可回退：
  - 提示词模板 DB 异常时回退会话级 / 请求级附加提示词直传能力。
...
| 2026-06-06 | 根据当前聊天提示词语义调整模板设计，明确 `prompt_template` 采用通用附加提示词和 `template_scope` 方案，并开始执行 P0 DDL/DML | `管理后台-task.md`、`docs/db/mysql-schema.sql`、`docs/db/table-relations.md` |
| 2026-06-07 | 接入后台鉴权首条 P2 链路，落地 HMAC token、PBKDF2 密码校验、前端登录态存储和 `/admin/**` 路由守卫 | `管理后台-task.md`、`application.yml`、`application-local.example.yml` |
| 2026-06-07 | 接入模型供应商、模型配置、提示词模板的后端主链路，并落地本地 AES-GCM API Key 加解密与模板驱动的全局附加提示词拼装 | `管理后台-task.md`、`application.yml`、`application-local.example.yml` |
| 2026-06-07 | 接入管理用户、普通用户、调用日志和用量统计的后端查询与状态流转逻辑 | `管理后台-task.md` |
